...
| Метод | ROC-AUC | PR-AUC | P@100 | NDCG@100 |
|---|---|---|---|---|
| **CLUE** | **0.992** | **0.809** | **1.000** | **1.000** |
| **NHFM** | **0.983** | **0.708** | 0.990 | 0.989 |
| LightGBM | 0.903 | 0.546 | 1.000 | 1.000 |
| CTKT | 0.894 | 0.068 | 0.260 | 0.246 |
| EthAegis | 0.848 | 0.018 | 0.050 | 0.022 |
| SPADE | 0.619 | 0.075 | 0.650 | 0.660 |
| GraphRfi | 0.545 | 0.084 | 1.000 | 1.000 |
| Logistic Regression | 0.931 | 0.132 | 0.000 | 0.000 |
| GiPM3F | 0.536 | 0.012 | 0.020 | 0.034 |

Последовательные нейросетевые методы (CLUE, NHFM) показывают наилучшие результаты, превосходя в том числе LightGBM. Это подтверждает что моделирование временной динамики поведения держателя карты является ключевым фактором для эффективного обнаружения мошенничества.
...
│   ├── notebook_2_baseline_ml.ipynb         # LR и LightGBM baseline
...
├── models/
│   ├── lgbm_model.pkl              # Обученная модель LightGBM
│   ├── lgbm_best_params.json       # Лучшие параметры Optuna
│   └── lr_pipeline.pkl             # Pipeline логистической регрессии
├── results/
│   ├── scores_*.csv                # Скоры каждого метода на test
│   ├── metrics_all.csv             # Сводная таблица метрик
...
| 2 | `notebook_2_baseline_ml` | LR + LightGBM + Optuna | ~15 мин |
...
| 6 | `notebook_6_evaluation` | Финальное сравнение и визуализации | ~5 мин |
...
- **F1** — баланс точности и полноты при оптимальном пороге

---

## Ключевые выводы

1. Последовательные нейросетевые методы (CLUE, NHFM) превосходят все остальные подходы — моделирование временной динамики поведения является ключевым для детекции фрода.
2. LightGBM с богатым feature engineering конкурентоспособен (PR-AUC=0.546) несмотря на отсутствие последовательного моделирования.
3. Графовые и матричные методы ограничены структурой синтетического датасета: 77.5% держателей имеют хотя бы одну мошенническую транзакцию в train, что делает узловые метки малоинформативными.
4. SPADE — единственный ненейросетевой метод — показывает умеренное качество (ROC-AUC=0.619), ограниченное отсутствием устойчивых персональных паттернов в синтетических данных.
...
- Cui et al. *CTKT: Collaborative Transaction Knowledge Transfer for Fraud Detection*. 2021.
- Yang et al. *Generalized Inverse Participation Ratio and Matrix Factorization for Fraud Detection (GiPM3F)*. 2018.